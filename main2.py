...
class MainWindow(QMainWindow):
    ...
    def create_post_card(self, post):
        ...
        # 添加封面图片
        if "thumb" in post and post["thumb"]:  # 确保封面图片存在
            cover_label = QLabel(card)
            cover_pixmap = QPixmap()
            cover_pixmap.loadFromData(requests.get(post["thumb"]).content)
            cover_pixmap = cover_pixmap.scaledToWidth(100, Qt.SmoothTransformation)  # 固定图片宽度
            cover_label.setPixmap(cover_pixmap)
            cover_label.setFixedWidth(100)  # 固定封面宽度
            cover_label.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Expanding)  # 高度随内容扩展
            card_layout.addWidget(cover_label)
    
        # 创建文本部分
        text_layout = QVBoxLayout()
        text_layout.setContentsMargins(10, 0, 0, 0)  # 调整文本与封面之间的间距
        ...
        card_layout.addLayout(text_layout)  # 将文本部分添加到主水平布局
